...
from preprocess import DATASET_PATH,CLASSES
import cv2
...
def convert_bbox2labels(bbox):
    ...
    gridsize = 1.0/7 # 归一化之后，每个grid的大小为1/7
...
class VOC2012(Dataset):
    def __init__(self,is_train=True,is_aug=True):
        ...
        if is_train:
            with open(DATASET_PATH + "\\ImageSets\\Main\\train.txt", 'r') as f: # 调用包含训练集图像名称的txt文件
...
if __name__ == '__main__':
    ...
    for idx, (img, label) in enumerate(dataloader):  # 解包img和label
        print(f"Image {idx}: {img.shape}")  # 打印图像的尺寸
        print(f"Label {idx}: {label.shape}")  # 打印标签
        # 如果想打印图像本身
        # print(img)
